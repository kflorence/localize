...
var slice = Array.prototype.slice,
    ...
    resourcePath = function( culture, resource ) {
      return localize.settings.paths.dictionary +
        ( culture || localize.culture ) +
        ( resource || "" ) + localize.settings.resources.extension;
    },
    ...
    localize = function( target, options ) {
      ...
      if ( !localize.initialized ) {
        localize.init( target, options );

      // Otherwise, it's just an alias for another localize function
      } else {
        switch ( typeof target ) {
          case "object": localize[ isElement( target ) ? "element" : "options" ]( target ); break;
          case "function": localize.ready( target ); break;
          case "string": return localize.key( target );
        }
      }
...
extend( localize, {
  ...
  init: function( options, callback ) {
    ...
    var resource = absolutePath( localize.uri.directory.replace( options.paths.root, "" ) ) +
        ( localize.uri.file.replace( options.patterns.extensions, "" ) || "index" ).toLowerCase(),
  ...
  element: function( element ) {
    if ( isElement( element ) && element.innerHTML ) {
      element.innerHTML = localize.parse( element.innerHTML );
    }
  ...
  options: function( options, returnOptions ) {
    ...
    each( options.paths, function( path, name ) {
      ...
      if ( name == "root" ) {
        ...
        if ( path == "./" ) {
          options.paths[ name ] = localize.uri.directory;
        }

      } else {
        options.paths[ path ] = absolutePath( options.paths.root, path );
      }
    });
  ...
  parse: function( culture, str ) {

    // Use current culture if one is not provided
    if ( str === undefined ) {
      str = culture;
      culture = localize.culture;
    }

    return str.replace( localize.settings.patterns.tag,

        // Replace all occurrences of the tag with the corresponding value
        // from the bundle, or leave it intact if a match is not found.
        function( matched, key, comment, offset, str ) {
          return localize.key( culture, key ) || matched;
        }) || str;
  },
