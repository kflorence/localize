...
extend( localize, {
  ...
  init: function( options, onReadyCallback ) {
    if ( localize.initialized ) {
      return;
    }
    ...
    onReadyCallback = onReadyCallback || options.ready;

    // Set up on localize ready callback
    if ( isFunction( options.callback ) ) {
      localize.ready( options.callback );
    }

    // Set up on DOM and localize ready callback
    if ( isFunction( onReadyCallback ) ) {
      localize.ready( onReadyCallback, true );
    }

    // Set up the require library for use with loading dictionary resources
    require.config({
    ...
    require( resources, function() {
      ...
      // Fire all waiting onReady callbacks
  ...
  // optionally executing only when the DOM is also ready
  ready: function( func, onDomReady ) {
    if ( isFunction( func ) ) {
      if ( localize.isReady ) {
        onDomReady ? require.ready( func ) : func();

      } else {
        onReadyCallbacks.push( onDomReady ? require.ready( func ) : func );
      }
    }
