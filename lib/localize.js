...
var window = global.window,
    ...
    // Assume browser environment if these properties exist
    isBrowser = !!( window && navigator && document && location ),

    // Keep track of whether or not the library has been initialized
    isInitialized = false,

    // Generic object/array iteration function (from underscore.js)
    ...
    // Merge two or more objects together (from jQuery and underscore.js)
...
extend( localize, {
  ...
  init: function( options, done, ready ) {
    ...
    var base = options.path.dictionary,
        extension = options.resource.extension,
        path = uri.directory.replace( options.path.root, "" ) || "/",
        resource = uri.file.toLowerCase() || options.resource.default,
        resources = [];
    ...
    each( options.cultures, function( culture ) {
      resources.push( absolutePath( base ) + culture + extension );
      resources.push( absolutePath( base, culture, path ) + resource + extension );
    });

    // Fire user-implemented resource loader
    if ( isFunction( options.load ) ) {
      options.load.call( global, resources, options );

    // Otherwise, use the built in method
    } else {
      localize.load( resources, function() {

        // Resources have loaded, fire the done callback, if provided
        if ( isFunction( options.done ) ) {
          options.done();
        }

        // On DOM ready, fire the ready callback, if provided
        if ( isFunction( options.ready ) ) {
          localize.ready( options.ready );
        }
      });
    }
  ...
  options: function( options, returnOptions ) {
    ...
    each( options.path, function( path, name ) {
      ...
      if ( name == "root" ) {
        ...
        options.path[ name ] = path.replace( "./", uri.directory );
  ...
  // Wrapper for require.ready()
  ready: function( callback ) {
    require.ready.apply( global, slice.call( arguments, 0 ) );
    return localize;
  },
